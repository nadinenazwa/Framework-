...
<div class="container">
    ...
    <form action="{{ route('resepsionis.pets.store') }}" method="POST">
        ...
        <div class="mb-3">
            <label for="idpemilik" class="form-label">Owner ID</label>
        ...
        <div class="mb-3">
            ...
            <select name="idras_hewan" id="idras_hewan" class="form-control">
                ...
                @foreach($breeds ?? [] as $b)
                    <option value="{{ $b->idras_hewan }}" {{ old('idras_hewan') == $b->idras_hewan ? 'selected' : '' }}>{{ $b->nama_ras }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="warna_tanda" id="warna_tanda" class="form-control" value="{{ old('warna_tanda') }}" maxlength="255">
